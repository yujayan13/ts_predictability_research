...
nmme = [0.256, 0.258, 0.225, 0.214, 0.237, 0.202]
cfsv2 = [0.183, 0.18, 0.09, 0.012, 0.087, 0.115]

num_models = 4
bar_width = 0.15
gap_within_groups = 0.05
inter_group_gap = 0.3
total_width = num_models * bar_width + (num_models - 1) * gap_within_groups
base_group_width = total_width + inter_group_gap

x_base_values = np.arange(len(linears)) * base_group_width

plt.figure(figsize=(12, 6))

plt.bar(x_base_values, cnns, width=bar_width, label='CNN', color=[0.031372549, 0.31372549, 0.588235294])
plt.bar(x_base_values + bar_width + gap_within_groups, linears, width=bar_width, label='MLR', color=[0.482352941, 0.7, 0.868627451])
plt.bar(x_base_values + 2 * (bar_width + gap_within_groups), nmme, width=bar_width, label='NMME', color='lightgreen')
plt.bar(x_base_values + 3 * (bar_width + gap_within_groups), cfsv2, width=bar_width, label='CFSv2', color='salmon')
...
plt.legend(prop={'family': 'Arial', 'size': 12}, loc="upper center")


plt.xticks(x_base_values + (3 / 2) * bar_width + gap_within_groups + gap_within_groups/2, np.arange(1, len(linears) + 1))

plt.yticks([0.00, 0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.35, 0.40], fontsize=12, fontname='Arial')

plt.xlim(-inter_group_gap, max(x_base_values) + base_group_width - inter_group_gap)
plt.tight_layout()
plt.savefig('correlation_bargraph_figure7_updated.png', dpi=600, bbox_inches='tight')
plt.show()
